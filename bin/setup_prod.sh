...
oc policy add-role-to-user edit system:serviceaccount:cicd-admin:jenkins -n tasks-prod
...
oc set probe dc/tasks-blue --readiness --get-url=http://:8080/ --initial-delay-seconds=50 --timeout-seconds=1 -n tasks-prod
oc set probe dc/tasks-blue --liveness --get-url=http://:8080/ --initial-delay-seconds=50 --timeout-seconds=1 -n tasks-prod
...
oc set probe dc/tasks-green --readiness --get-url=http://:8080/ --initial-delay-seconds=50 --timeout-seconds=1 -n tasks-prod
oc set probe dc/tasks-green --liveness --get-url=http://:8080/ --initial-delay-seconds=50 --timeout-seconds=1 -n tasks-prod
# Setting 'wrong' VERSION. This will need to be updated in the pipeline
oc set env dc/tasks-green VERSION='0.0 (tasks-green)' -n tasks-prod
